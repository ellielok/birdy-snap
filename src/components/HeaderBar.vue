<template>
    <header class="w-full h-15 bg-[rgba(40,101,201,0.93)] flex items-center justify-between px-8 max-lg:px-4 relative z-100 box-border">
        ...
        <!-- Desktop nav -->
        <div class="hidden lg:flex items-center gap-4.5">
                <button class="bg-[#1976d2] text-white border-none rounded-2xl py-1.75 px-5 text-base cursor-pointer ml-3 transition-colors duration-200 hover:bg-[#145ea8]" @click="goToImageSearch">
                    Reverse Search
                </button>
        ...
        <button class="lg:hidden flex flex-col justify-center items-center gap-1.5 bg-transparent border-none cursor-pointer p-2" @click="menuOpen = !menuOpen" aria-label="Toggle menu">
            <span class="block w-6 h-0.75 bg-white rounded transition-all duration-300" :class="{ 'rotate-45 translate-y-2': menuOpen }"></span>
            <span class="block w-6 h-0.75 bg-white rounded transition-all duration-300" :class="{ 'opacity-0': menuOpen }"></span>
            <span class="block w-6 h-0.75 bg-white rounded transition-all duration-300" :class="{ '-rotate-45 -translate-y-2': menuOpen }"></span>
        </button>

        <!-- Mobile dropdown menu -->
        <div v-show="menuOpen" class="lg:hidden absolute top-15 left-0 w-full bg-[rgba(40,101,201,0.97)] backdrop-blur-sm shadow-lg z-99 flex flex-col py-2 transition-all duration-250 ease-out origin-top" :class="menuOpen ? 'opacity-100 scale-y-100' : 'opacity-0 scale-y-0'">
